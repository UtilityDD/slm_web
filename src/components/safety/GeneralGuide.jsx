...
const steps = [
    ...
    },
    {
        ...
        instruction: { en: 'Please listen to the oath. Your family is waiting for you.', bn: 'শান্ত হয়ে অঙ্গীকারটি শুনুন। মনে রাখবেন, আপনার পরিবার আপনার অপেক্ষায় আছে।' },
        icon: '🗣️',
    ...
    {
        id: 'pre_checklist',
        title: { en: 'Equipment Check', bn: 'সরঞ্জাম মিলিয়ে নিন' },
        instruction: { en: 'Is all your gear ready? Please check one by one.', bn: 'সব সরঞ্জাম ঠিক আছে তো? এক এক করে মিলিয়ে নিন।' },
        icon: '✅',
    ...
    },
    {
        ...
        instruction: {
            ...
            bn: 'অসাধারণ! সরঞ্জাম সব ঠিক আছে। এবার আমাদের সবচেয়ে গুরুত্বপূর্ণ কাজ—এলাকাটি আইসোলেট করা। সাবধানে দুদিক থেকে পাওয়ার আসা বন্ধ করুন এবং প্রয়োজনীয় জায়গায় ব্যারিকেড দিন।'
        },
        icon: '🚧',
    ...
    {
        id: 'work',
        title: { en: 'During Work', bn: 'কাজের সময়' },
        instruction: { en: 'Stay focused. Keep your safety belt and grounding tight.', bn: 'মনোযোগ দিয়ে কাজ করুন। বেল্ট এবং গ্রাউন্ডিং সবসময় সাথে রাখুন।' },
        icon: '⚡',
    ...
    },
    {
        ...
        instruction: {
            ...
            bn: 'সাবাশ! কাজ সফলভাবে শেষ হয়েছে। এবার লাইনটি আবার চালু করার আগে শেষবারের মতো নিশ্চিত হয়ে নিন যে সব গ্রাউন্ডিং সরানো হয়েছে এবং কোনো সরঞ্জাম লাইনে ফেলে রাখা হয়নি।'
        },
        icon: '🔍',
    ...
    },
    {
        ...
        instruction: { en: 'Great job. Mission completed safely. You are now ready for your next task or to head back.', bn: 'দারুণ কাজ! মিশনটি নিরাপদে সম্পন্ন হয়েছে। এখন আপনি পরবর্তী কাজের জন্য বা ফিরে যাওয়ার জন্য প্রস্তুত।' },
        icon: '🏆',
...
const ISOLATION_CHECKLIST_ITEMS = [
    { id: 'isolate', label: { en: 'Isolate Both Sides', bn: 'দুদিক থেকে আইসোলেট করা হয়েছে' }, icon: '🔌', audio: 'isolated.wav' },
    { id: 'test', label: { en: 'Tested Voltage', bn: 'ভোল্টেজ চেক করা হয়েছে' }, icon: '📟', audio: 'tested.wav' },
    { id: 'discharge', label: { en: 'Line Discharged', bn: 'লাইন ডিসচার্জ করা হয়েছে' }, icon: '⚡', audio: 'discharged.wav' },
    { id: 'grounding', label: { en: 'Earth/Grounding Done', bn: 'শর্টিং ও গ্রাউন্ডিং করা হয়েছে' }, icon: '🦯', audio: 'grounded.wav' },
    { id: 'barricade', label: { en: 'Site Barricaded', bn: 'ব্যারিকেড নিশ্চিত করা হয়েছে' }, icon: '🚧', audio: 'barricaded.wav' }
];

const POST_CHECKLIST_ITEMS = [
    { id: 'ground_rem', label: { en: 'Grounding Removed', bn: 'সব গ্রাউন্ডিং সরানো হয়েছে' }, icon: '⚡', audio: 'ground_removed.wav' },
    { id: 'tools_counted', label: { en: 'Tools Counted', bn: 'সব সরঞ্জাম গুনে নেওয়া হয়েছে' }, icon: '🧰', audio: 'tools_counted.wav' },
    { id: 'site_clean', label: { en: 'Site Cleaned', bn: 'কাজের জায়গা পরিষ্কার করা হয়েছে' }, icon: '🧹', audio: 'site_clean.wav' },
    { id: 'permit_return', label: { en: 'Permit Returned', bn: 'পারমিট (PTW) ফেরত দেওয়া হয়েছে' }, icon: '📝', audio: 'permit_return.wav' },
    { id: 'barricade_rem', label: { en: 'Barricade Removed', bn: 'ব্যারিকেড সরানো হয়েছে' }, icon: '🚧', audio: 'barricade_removed.wav' }
];

export default function GeneralGuide({ language = 'bn', onClose }) {
    ...
    const handleNext = () => {
        if (currentStep.id === 'pre_checklist') {
            ...
            if (!preItemsChecked) {
                playAlertSound();
                playAudio(language === 'bn' ? 'দয়া করে আগে সব সরঞ্জাম চেক করুন' : 'Please check all items first', 'alert_check.mp3');
                return;
            }
        }
        ...
        if (currentStep.id === 'isolation') {
            ...
            if (!isoItemsChecked) {
                playAlertSound();
                playAudio(language === 'bn' ? 'দয়া করে আইসোলেশন ও গ্রাউন্ডিং নিশ্চিত করুন' : 'Please confirm isolation and grounding', 'alert_isolate.wav');
                return;
            }
        }
        ...
        if (currentStep.id === 'final_check') {
            ...
            if (!postItemsChecked) {
                playAlertSound();
                playAudio(language === 'bn' ? 'আগে নিশ্চিত করুন সব গ্রাউন্ডিং সরানো হয়েছে কিনা' : 'Please confirm everything is cleared', 'alert_done.mp3');
                return;
            }
        }
    ...
    const getContinueBtnText = () => {
        if (language === 'en') {
            ...
            switch (currentStep.id) {
                ...
                case 'isolation': return 'হ্যাঁ, আইসোলেট করা হয়েছে';
                case 'work': return 'কাজ শেষ হলো?';
                case 'final_check': return 'সব সম্পন্ন হয়েছে?';
                ...
                {viewMode === 'history' ? (
                    <div className="space-y-4 animate-slide-up">
                        <div className="mb-6">
                            ...
                            <p className="text-[10px] text-orange-500 font-bold px-1 italic">"আপনার সুরক্ষা আপনার পরিবারের সবচেয়ে বড় সম্পদ।"</p>
                        </div>
                        {history.length === 0 ? (
                            <div className="py-32 text-center space-y-4">
                                <div className="text-6xl opacity-20">📂</div>
                                <p className="text-slate-400 font-bold">সাথী এখনো কোনো রেকর্ড পায়নি।<br/>নিরাপদে কাজ শুরু করুন!</p>
                            </div>
                        ) : (
                            history.map(log => (
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-center p-4 space-y-10 animate-slide-up">
                                ...
                                <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-xl border border-slate-200 dark:border-slate-800">
                                    <p className="text-xl sm:text-2xl font-black text-slate-800 dark:text-white leading-tight italic">
                                        {language === 'bn'
                                            ? "আমি অঙ্গীকার করছি যে, আমি সমস্ত নিয়ম মেনে চলব। আমার সুরক্ষা এবং আমার পরিবারের সুখ আমার হাতে।"
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center space-y-12 animate-slide-up">
                                <div className="relative w-64 h-64 flex items-center justify-center bg-white dark:bg-slate-900 rounded-full shadow-2xl border-[16px] border-amber-500/20">
                                    ...
                                    <div className="text-center">
                                         <p className="text-4xl font-black text-slate-800 dark:text-white">মন দিয়ে কাজ করুন</p>
                                ...
                                <div className="p-6 bg-amber-100 dark:bg-amber-900/30 rounded-3xl border border-amber-200 text-amber-800 dark:text-amber-400 font-bold text-center">
                                    সাবধান! কাজ শেষ না হওয়া পর্যন্ত গ্রাউন্ডিং সরাবেন না।
                                </div>
                            </div>
                        )}
                        ...
                        {currentStep.id === 'done' && (
                            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-10 animate-scale-in">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-500 dark:text-slate-400 font-bold">আপনার সুরক্ষার রেকর্ড সাথী যত্ন করে রেখে দিয়েছে।</p>
                                </div>
                            </div>
                        )}
                    </div>
