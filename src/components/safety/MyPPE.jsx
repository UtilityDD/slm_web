...
const MyPPE = ({ user, setCurrentView, language }) => {
    ...
    const missingEssentials = ppeChecklist.filter(item => item.essential && !item.available);
    const myGear = ppeChecklist.filter(item => item.available);
    const otherAvailable = ppeChecklist.filter(item => !item.essential && !item.available);

    const PPECard = ({ item, index, compact = false }) => (
        <div
            ...
            <div className="sticky top-0 z-20 bg-slate-50/90 dark:bg-slate-900/90 backdrop-blur-sm py-4 border-b border-slate-200 dark:border-slate-800 -mx-4 px-4 flex justify-between items-center">
                <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                        {myGear.length}/{PPE_ITEMS.length} Items • {missingEssentials.length} Missing Essentials
                    </p>
                </div>
                {!user ? (
                        ...
                        <div className="bg-red-50 dark:bg-red-900/10 rounded-2xl p-4 border border-red-100 dark:border-red-900/30">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {missingEssentials.map(item => {
                                    const realIndex = ppeChecklist.findIndex(p => p.name === item.name);
                                    return <PPECard key={item.name} item={item} index={realIndex} />;
                                })}
                            </div>
                        </div>
                    )}
