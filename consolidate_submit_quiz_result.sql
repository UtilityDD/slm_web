-- 1. DROP all known overloads to clear ambiguity
DROP FUNCTION IF EXISTS submit_quiz_result(text, integer);
DROP FUNCTION IF EXISTS submit_quiz_result(text, integer, integer);
DROP FUNCTION IF EXISTS submit_quiz_result(text, integer, integer, uuid);

-- 2. CREATE the authoritative version (handling breakdown columns)
