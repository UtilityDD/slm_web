/**
 * Build HTML review for visual_quiz_batch_swy33_parts_preview.csv
 * Usage: node scripts/maintenance/build_swy33_parts_preview_html.mjs
 */
import fs from 'fs';
...
const inputPath = path.join(root, 'quiz_management', 'visual_quiz_batch_swy33_parts_preview.csv');
const outPaths = [
  path.join(root, 'quiz_management', 'visual_quiz_swy33_parts_preview.html'),
  path.join(root, 'public', 'quiz_management', 'visual_quiz_swy33_parts_preview.html'),
];
  ...
  .map((q) => {
    ...
    <article class="card" id="${esc(q.id)}">
      <div class="meta">
        ...
        <span class="tag warn">DRAFT — 33kV outdoor switchyard</span>
      </div>
      ${img}
      <h2>${esc(q.question_text)}</h2>
      ${opts}
      <p class="hint"><strong>ইঙ্গিত:</strong> ${esc(q.hint)}</p>
    </article>`;
  })
  .join('\n');
...
<head>
  ...
  <title>33kV outdoor switchyard parts quiz review — ${questions.length} questions</title>
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
  <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+Bengali:wght@400;600;700&display=swap" rel="stylesheet" />
  <style>
    body { font-family: "Noto Sans Bengali", "Hind Siliguri", "Segoe UI", system-ui, sans-serif; margin: 0; background: #f8fafc; color: #0f172a; }
...
<body>
  <header>
    <h1>33kV outdoor switchyard parts — manual review</h1>
    <p>Source: <code>visual_quiz_batch_swy33_parts_preview.csv</code> · <strong>${questions.length}</strong> questions · Draft only.</p>
    <p>Open: <code>http://localhost:5173/quiz_management/visual_quiz_swy33_parts_preview.html</code></p>
    <nav>
      ${questions.map((q) => `<a href="#${esc(q.id)}">${esc(q.id.replace('vq-swy33-', ''))}</a>`).join('')}
    </nav>
  </header>
  <main>
${cards}
...
fs.copyFileSync(
  inputPath,
  path.join(root, 'public', 'quiz_management', 'visual_quiz_batch_swy33_parts_preview.csv')
);
