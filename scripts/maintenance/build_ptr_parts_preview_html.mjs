/**
 * Build HTML review for visual_quiz_batch_ptr_parts_preview.csv
 * Usage: node scripts/maintenance/build_ptr_parts_preview_html.mjs
 */
import fs from 'fs';
...
const inputPath = path.join(root, 'quiz_management', 'visual_quiz_batch_ptr_parts_preview.csv');
const outPaths = [
  path.join(root, 'quiz_management', 'visual_quiz_ptr_parts_preview.html'),
  path.join(root, 'public', 'quiz_management', 'visual_quiz_ptr_parts_preview.html'),
];
  ...
  .map((q) => {
    ...
    <article class="card" id="${esc(q.id)}">
      <div class="meta">
        ...
        <span class="tag warn">DRAFT — PTR parts</span>
      </div>
      ${img}
...
<head>
  ...
  <title>PTR parts ID quiz review — ${questions.length} questions</title>
  <style>
...
<body>
  <header>
    <h1>PTR parts identification — manual review</h1>
    <p>Source: <code>visual_quiz_batch_ptr_parts_preview.csv</code> · <strong>${questions.length}</strong> questions · Draft only.</p>
    <p>Open: <code>http://localhost:5173/quiz_management/visual_quiz_ptr_parts_preview.html</code></p>
    <nav>
      ${questions.map((q) => `<a href="#${esc(q.id)}">${esc(q.id.replace('vq-ptr-', ''))}</a>`).join('')}
    </nav>
  </header>
  <main>
${cards}
...
fs.copyFileSync(
  inputPath,
  path.join(root, 'public', 'quiz_management', 'visual_quiz_batch_ptr_parts_preview.csv')
);
