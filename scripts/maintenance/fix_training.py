...
# Match the div container that has roadmapItems.map inside it
# We'll use a very specific pattern to avoid matching other divs
pattern = re.compile(r'<div className="relative z-10">\s*\{roadmapItems\.map\(\(item, index\) => \{.*?\}\);\s*\}\)\s*</div>', re.DOTALL)

if pattern.search(content):
    new_content = pattern.sub(new_block, content)
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully replaced with pattern 1")
else:
    # Try another pattern if the first one fails
    pattern2 = re.compile(r'<div className="relative z-10">\s*\{roadmapItems\.map\(\(item, index\) => \{.*?isLocked.*?\}\);\s*\}\)\s*</div>', re.DOTALL)
    if pattern2.search(content):
        new_content = pattern2.sub(new_block, content)
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("Successfully replaced with pattern 2")
    else:
