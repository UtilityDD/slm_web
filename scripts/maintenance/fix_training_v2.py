...
foothold = "// FOOTHOLD_START"
if foothold not in content:
    print("Foothold not found")
    exit(1)

start_idx = content.find(foothold)

# Find the next '})}' after the foothold
# We'll look for the map closing pattern
end_marker = "})}"
end_pos = content.find(end_marker, start_idx)

if end_pos == -1:
    print("End marker not found")
    exit(1)

# Include the marker itself in the replacement range
# Wait, we want to replace from FOOTHOLD_START to the end of that line which is line 1317.
end_idx = end_pos + len(end_marker)

new_block = """                                        {roadmapItems.map((item, index) => {
