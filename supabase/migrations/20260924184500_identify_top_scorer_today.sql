-- Gift/rules plaque: today's IST top awarded points (cap 100), not all-time last run.
...
COMMENT ON FUNCTION public.get_identify_top_scorer() IS
  'Identify rules card: today''s highest awarded Parichiti points (IST, cap 100) + name.';
