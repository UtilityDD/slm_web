-- Progressive profile nudge: store prompt cadence/skips + allow custom-auth
-- clients to update a whitelist of demographic fields (auth.uid() is null).

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS profile_nudge_state jsonb NOT NULL DEFAULT '{}'::jsonb;

COMMENT ON COLUMN public.profiles.profile_nudge_state IS
  'Progressive profile nudge state: { last_prompt_date, skips: { field: count } }';
...
DECLARE
    ...
    v_allowed text[] := ARRAY[
        'district', 'block', 'job', 'dob', 'age', 'education',
...
BEGIN
    ...
    -- Build dynamic SET for whitelisted keys only
