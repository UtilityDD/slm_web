...
**Egress:** do not call `fetchAllTime` / `fetchMonthly` from login, Home, or pull-to-refresh. Rank loads when Rank/Prizes opens. All-time still runs **`overlayCumulativeReading`** (paginated reading attempts for the top 50) — required until `profiles.reading_points` includes day-stamped re-reads. See **[`docs/developer-guides/free-plan-optimization.md`](docs/developer-guides/free-plan-optimization.md)**.
...
- **All-time:** `leaderboard_view` → `points` / `reading_points` (see `leaderboardService.fetchAllTime`). Display overlay **`overlayCumulativeReading`** still pages reading `quiz_attempts` for the top 50 — needed because profile RDG often stops at first-time lesson credits.
...
- **2026-09:** Pull-to-refresh no longer prefetches Rank. HoF **v11** + closed-month snapshots. Keep All-time **`overlayCumulativeReading`** (dropping it showed first-time-only RDG). See **`free-plan-optimization.md`**.
